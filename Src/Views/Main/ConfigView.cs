...
internal sealed class ConfigView : UserControl, IConfigView
{
    public event Action<ModManagerConfig>? ApplyConfigurationRequested;
    public event Action? AutoDetectGameDirectoryRequested;

    /// <summary>Replace the value of the game-path field (called by the presenter after auto-detect).</summary>
        ...
        Action<string>? setStatus)
    {
        ...
        saveButton.Click += (_, _) =>
        {
            ...
            // Phase 4e: prefer the IConfigView event if a presenter is wired;
            // fall back to the legacy callback otherwise.
            if (ApplyConfigurationRequested is { } handler)
            {
                handler(config);
            }
            else
            {
                applyConfiguration?.Invoke(config);
            }
        };
    ...
    private void BrowseForGamePath()
    {
        ...
        if (dialog.ShowDialog(this) == DialogResult.OK)
        {
            ...
            setStatus?.Invoke(loc.Get("game.game_path_browsed_status", dialog.SelectedPath));
        }
    }
    ...
    private void AutoDetectGamePath()
    {
        try
        {
            // If a presenter is wired, route through the event so it can
            // call LaunchService and push the result back via SetGameDirectory.
            if (AutoDetectGameDirectoryRequested is { } handler)
            {
                handler();
                setStatus?.Invoke(loc.Get("game.game_path_detected_status", gamePathTextBox.Text));
                return;
            }

            gamePathTextBox.Text = autoDetectGameDirectory?.Invoke() ?? string.Empty;
            setStatus?.Invoke(loc.Get("game.game_path_detected_status", gamePathTextBox.Text));
        }
