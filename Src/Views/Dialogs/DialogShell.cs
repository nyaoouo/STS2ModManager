...
internal class DialogShell : MaterialForm
{
    ...
    protected DialogShell(string title, int contentWidth, int contentHeight)
    {
        ...
        var outer = new TableLayoutPanel
        {
            ...
            RowCount = 2,
            Dock = DockStyle.Fill,
            // Top padding clears MaterialForm's action-bar/title chrome (~32-36 px).
            Padding = new Padding(20, 36, 20, 16),
        };
        ...
        Controls.Add(outer);
        // Outer padding (top 36 + bottom 16) + ContentHost bottom padding (12) +
        // button-bar height (~28) + a small safety margin so AutoSize labels never clip.
        ClientSize = new Size(
            Math.Max(contentWidth + 40, 320),
            contentHeight + 36 + 16 + 12 + 28 + 8);
    }
